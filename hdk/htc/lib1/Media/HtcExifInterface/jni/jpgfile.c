...
// >>> HTC 3D Macro Check End of file for HTC tag, avoid image editor remove unknown block.
                        
                        /* HTC User Comment Prefix: hTC*/
                        /* Macro 3D: hTC_3D*/
                        if ((ImageInfo.Comments[0] == 0x68) 
                            && (ImageInfo.Comments[1] == 0x54)
                            && (ImageInfo.Comments[2] == 0x43)) 
                        {
                            if ((ImageInfo.Comments[3] == 0x5F)
                                && (ImageInfo.Comments[4] == 0x33)
                                && (ImageInfo.Comments[5] == 0x44))
                            {
                                const char bytes[] = { 0x68, 0x74, 0x63, 0x00, 0x74, 0x77, 0x74, 0x70 };
                                ImageInfo.Is3DMacro = verifyHtcFormat(infile, -1, bytes, sizeof(bytes));
                            }
                        }
// <<< HTC 3D Macro

// >>> HTC Bokeh
                        if (ImageInfo.DualCam == 300) {
                            const char bytes[] = { 0x55, 0x46, 0x43, 0x53 };  //UFCS
                            ImageInfo.IsBokeh = verifyHtcFormat(infile, -1, bytes, sizeof(bytes));
                        }
// <<< HTC Bokeh
...
// >>> HTC Panorama+
                        // Leon_Chu@htc.com: Check if this xmp is Google Panorama format.
                        ImageInfo.IsPanorama = checkIsGPano( (char const*) Sections[SectionsRead-1].Data, Sections[SectionsRead-1].Size );
...
// Read image data.
//--------------------------------------------------------------------------
int ReadJpegFileFromFD(const int fd, ReadMode_t ReadMode){
    
    FILE * infile;
    infile = fdopen(fd, "rb"); // Unix ignores 'b', windows needs it.

    return readJpegFileInternal(infile, ReadMode);
}
...
int readJpegFileInternal(FILE *infile, ReadMode_t ReadMode)
{
    int ret;

    if (infile == NULL) {
        ALOGE("can't open file");
    ...
    if (!ret){
        ALOGV("Cannot parse JPEG sections for file");
