...
static void show_tag(ExifData *d, ExifIfd ifd, ExifTag tag)
{
	...
	printf("photographer.c : entry == null for tag: %0x\n",tag);//add by qqzhong for log
    }
}
...
static void show_mnote_tag(ExifData *d, unsigned tag)
{
    ...
    if (mn) {
        ...
        //printf("ExifMnoteData data value num is : %d\n",num);//add by qqzhong for log
            ...
            if (exif_mnote_data_get_id(mn, i) == tag) {
                if (exif_mnote_data_get_value(mn, i, buf, sizeof(buf))) {
                    ...
                    if (*buf) {
                       ...
                       // printf("%s: %x\n", exif_mnote_data_get_title(mn, i),buf); //add by qqzhong for log
                    }
                }
            }
        }
    }else{
        printf("ExifMnoteData == null\n");//add by qqzhong for log
    }
}
...
//   show_tag(ed, EXIF_IFD_0, EXIF_TAG_COPYRIGHT);
    ...
    show_tag(ed, EXIF_IFD_0, EXIF_TAG_MAKE);//add by qqzhong for log
    //show_tag(ed, EXIF_IFD_0, EXIF_TAG_USER_COMMENT);//add by qqzhong for log
...
//		show_mnote_tag(ed, 7);
    ...
    }else{            //add by qqzhong for log
	//printf("photographer.c : entry ==null\n");     //add by qqzhong for log
    }                 //add by qqzhong for log
