...
#include <libexif/htc/exif-mnote-data-htc.h>
...
interpret_maker_note(ExifData *data, const unsigned char *d, unsigned int ds)
{
	 ...
	 * heuristics are pretty general. */
		...
		data->priv->md = exif_mnote_data_pentax_new (data->priv->mem);
           //add by zhongqiqiang begin for htc
	} else if ((mnoteid = exif_mnote_data_htc_identify (data, e)) != 0) {
		exif_log (data->priv->log, EXIF_LOG_CODE_DEBUG,
			"ExifData", "Htc MakerNote variant type %d", mnoteid);
		data->priv->md = exif_mnote_data_htc_new (data->priv->mem);
	}
	  //add by zhongqiqiang end for htc
