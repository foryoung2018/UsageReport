...
//
/*
 * @see exif_entry_get_value
 *
 * support raw data output,so we can parse data self if need
 *
 * \param[out] raw_data_out make suer free raw data in invoker function
 * \param[in] raw data buffer length
 */
const char *exif_entry_get_value_super (ExifEntry *entry, char *val,
		  unsigned int maxlen,char** attrs_out,int attrs_buf_len);
//------------add_by_shenkaige
