...
{
	return exif_entry_get_value_super(e,val,maxlen,NULL,0);
}
const char *exif_entry_get_value_super (ExifEntry *e, char *val, unsigned int maxlen,char** attrs,int buflen){
	unsigned int i, j, k;
	...
	case EXIF_TAG_MAX_APERTURE_VALUE:
		...
		if(attrs){
			buflen = addKeyValueDouble(attrs,buflen,ATTR_APERTURE_VALUE,pow (2, d / 2.),"%f");
			buflen = addKeyValueDouble(attrs,buflen,ATTR_EXPOSURE_VALUE,d,"%f");
		}
		//add_by_shenkaige----finish
		break;
	case EXIF_TAG_FOCAL_LENGTH:
		CF (e, EXIF_FORMAT_RATIONAL, val, maxlen);
		CC (e, 1, val, maxlen);
		...
		if (d){
			int equivalent35 = (int) (d * (double) v_rat.numerator / (double) v_rat.denominator);
			snprintf (b, sizeof (b), _(" (35 equivalent: %d mm)"),equivalent35);
			//add_by_shenkaige----end
			if(attrs){
				buflen = addKeyValueInt(attrs,buflen,ATTR_FOCAL_LENGTH_35_EQUIVALENT,equivalent35);
			}
			//add_by_shenkaige----finish
		}
		d = (double) v_rat.numerator / (double) v_rat.denominator;
		...
		//add_by_shenkaige----end
		if(attrs){
			buflen = addKeyValueDouble(attrs,buflen,ATTR_FOCAL_LENGTH,d,"%f");
		}
		//add_by_shenkaige----finish
		break;
	case EXIF_TAG_SUBJECT_DISTANCE:
		...
		//add_by_shenkaige----end
		if(attrs){
			buflen = addKeyValueDouble(attrs,buflen,ATTR_SUBJECT_DISTANCE,d,"%f");
		}
		//add_by_shenkaige----finish
		break;
	case EXIF_TAG_EXPOSURE_TIME:
		...
		//add_by_shenkaige----end
		if(attrs){
			buflen = addKeyValueDouble(attrs,buflen,ATTR_EXPOSURE_TIME,d,"%f");
		}
		//add_by_shenkaige----finish
		break;
	case EXIF_TAG_SHUTTER_SPEED_VALUE:
		...
		if(attrs){
			buflen = addKeyValueDouble(attrs,buflen,ATTR_APERTURE_VALUE,d,"%f");
		}
		//add_by_shenkaige----end
		d = 1. / pow (2, d);
		if (d < 1)
		...
		//add_by_shenkaige----start
		if(attrs){
			buflen = addKeyValueDouble(attrs,buflen,ATTR_SHUTTER_SPEED,d,"%f");
		}
		//add_by_shenkaige----end
		break;
	case EXIF_TAG_BRIGHTNESS_VALUE:
		...
		//add_by_shenkaige----end
		d = 1. / (M_PI * 0.3048 * 0.3048) * pow (2, d);
		snprintf (b, sizeof (b), _(" (%.02f cd/m^2)"),d);
		if (maxlen > strlen (val) + strlen (b))
			strncat (val, b, maxlen - strlen (val));
		//add_by_shenkaige----start
		if(attrs){
			buflen = addKeyValueDouble(attrs,buflen,ATTR_BRIGHTNESS_VALUE,d,"%f");
		}
		//add_by_shenkaige----end
		break;
	...
	case EXIF_TAG_SUBJECT_AREA:
		...
		case 3:
			...
			//add_by_shenkaige----start
			if(attrs){
				buflen = addKeyValueInt(attrs,buflen,ATTR_SUBJECT_AREA_RECTANGLE,v_short3);
			}
			//add_by_shenkaige----end
			break;
		case 4:
			...
			//add_by_shenkaige----start
			if(attrs){
				buflen = addKeyValueInt(attrs,buflen,ATTR_SUBJECT_AREA_WIDTH,v_short3);
				buflen = addKeyValueInt(attrs,buflen,ATTR_SUBJECT_AREA_HEIGHT,v_short4);
			}
			//add_by_shenkaige----end
			break;
		default:
		...
		//add_by_shenkaige----start
		if(attrs){
			buflen = addKeyValueInt(attrs,buflen,ATTR_SUBJECT_AREA_X,v_short);
			buflen = addKeyValueInt(attrs,buflen,ATTR_SUBJECT_AREA_Y,v_short2);
		}
		//add_by_shenkaige----end
		break;
	...
	case EXIF_TAG_GPS_TIME_STAMP:
		...
		if(attrs){
			addKeyValueInt(attrs,buflen,ATTR_DATE_HOUR,(int)i);
			addKeyValueInt(attrs,buflen,ATTR_DATE_MINUTE,(int)j);
			addKeyValueDouble(attrs,buflen,ATTR_DATE_SECOND_FLOAT,d,"%05.2f");
		}
		break;
	...
	case EXIF_TAG_DATE_TIME_ORIGINAL:
		if(attrs){
			buflen = addKeyValueString(attrs,buflen,ATTR_TIMESTAMP,e->data);
		}
		exif_entry_format_value(e, val, maxlen);
		break;
	case EXIF_TAG_GPS_DATE_STAMP:
		exif_entry_format_value(e, val, maxlen);
		if(attrs){
			buflen = addKeyValueString(attrs,buflen,ATTR_DATESTAMP,e->data);
		}
		break;
		//add_by_shenkaige----end
...
{
	if(exif_entry_patch_initialize(e,tag,e->priv->mem)){
		return;
	}
	ExifRational r;
	ExifByteOrder o;
