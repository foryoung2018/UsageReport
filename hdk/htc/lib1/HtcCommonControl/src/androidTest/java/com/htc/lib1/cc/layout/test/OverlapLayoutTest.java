...
import com.htc.lib1.cc.layout.activityhelper.HtcOverlapLayoutActivity;
...
public class OverlapLayoutTest extends HtcActivityTestCaseBase {
    ...
    private void test(boolean translucentStatusBar, boolean hasStatusBar, boolean hasActionBar,
            boolean translucentNavigationBar, boolean hasNavigationBar, boolean hasIME) {
        Intent i = new Intent();
        ...
        if (!hasNavigationBar) {
            mSolo.waitForActivity(HtcOverlapLayoutActivity.class);
        }

        ViewUtil.AssertViewSizeAndLocationEqualBefore(mSolo,
                mSolo.getView(R.id.testTextView), this);
    }
    ...
    private void testFooter(int footerDisplayMode) {
        ...
        ScreenShotUtil.AssertViewEqualBefore(mSolo,
                mSolo.getView(R.id.testHtcFooter), this);
    }
    ...
    public final void testImproveCoverage() {
        ...
        try {
            runTestOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ((HtcOverlapLayoutActivity) mActivity).improveCoverage();
                }
            });
        } catch (Throwable e) {
            e.printStackTrace();
