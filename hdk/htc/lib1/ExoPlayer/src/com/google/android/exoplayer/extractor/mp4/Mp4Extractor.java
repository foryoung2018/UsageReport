...
import com.htc.lib1.exo.extractor.mp4.HtcAtomParsers;
...
public final class Mp4Extractor implements Extractor, SeekMap {
  ...
  private void processMoovAtom(ContainerAtom moov) {
    ...
    for (int i = 0; i < moov.containerChildren.size(); i++) {
      ...
      if (atom.type != Atom.TYPE_trak && atom.type != Atom.TYPE_htka) {
        continue;
      }
      //[htc]Add by OOD, for SlowMotion, start
      Track track = HtcAtomParsers.parseTrak(atom, moov.getLeafAtomOfType(Atom.TYPE_mvhd));
      //[htc]Add by OOD, for SlowMotion, end
  ...
  private static boolean shouldParseLeafAtom(int atom) {
    return atom == Atom.TYPE_mdhd || atom == Atom.TYPE_mvhd || atom == Atom.TYPE_hdlr
        ...
        || atom == Atom.TYPE_stco || atom == Atom.TYPE_co64 || atom == Atom.TYPE_tkhd || atom == Atom.TYPE_htka;
  }

  /** Returns whether the extractor should parse a container atom with type {@code atom}. */
  private static boolean shouldParseContainerAtom(int atom) {
    return atom == Atom.TYPE_moov || atom == Atom.TYPE_trak || atom == Atom.TYPE_mdia
        || atom == Atom.TYPE_minf || atom == Atom.TYPE_stbl || atom == Atom.TYPE_htka;
  }
