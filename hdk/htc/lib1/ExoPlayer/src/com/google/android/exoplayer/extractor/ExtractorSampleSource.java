...
public class ExtractorSampleSource implements SampleSource, ExtractorOutput, Loader.Callback {
  ...
  public boolean prepare(long positionUs) throws IOException {
    ...
    if (seekMap != null && tracksBuilt && haveFormatsForAllTracks()) {
      ...
      for (int i = 0; i < trackCount; i++) {
        ...
        trackInfos[i] = new TrackInfo(format.mimeType, format.durationUs, format /* htc designed*/);
  ...
  private ExtractingLoadable createLoadableFromStart() {
    return new ExtractingLoadable(uri, dataSource, extractor, allocator, requestedBufferSize, 0, 0/* Htc defined */);
  }
  ...
  private ExtractingLoadable createLoadableFromPositionUs(long positionUs) {
    return new ExtractingLoadable(uri, dataSource, extractor, allocator, requestedBufferSize,
        seekMap.getPosition(positionUs), positionUs/* Htc defined */);
  }
  ...
  private static class ExtractingLoadable implements Loadable {
    ...
    public ExtractingLoadable(Uri uri, DataSource dataSource, Extractor extractor,
        DefaultAllocator allocator, int requestedBufferSize, long position, long positionInUs /* Htc defined*/) {
      ...
      //[htc]Add by OOD, for DLNA time seek, start
      positionHolder.positionInUs = positionInUs;
      //[htc]Add by OOD, for DLNA time seek, start
    ...
    public void load() throws IOException, InterruptedException {
      ...
      while (result == Extractor.RESULT_CONTINUE && !loadCanceled) {
        ...
        try {
          ...
          //[htc]Add by OOD, for DLNA time seek, start
          DataSpec dataSpec = new DataSpec(uri, position, position, C.LENGTH_UNBOUNDED, null, 0 , positionHolder.positionInUs);
          LOG.I(TAG, "load dataSpec = " + dataSpec.toString());
          long length = dataSource.open(dataSpec);
          if(dataSource instanceof DLNADataSource)
          {
        	  DLNADataSource dlnaSource = (DLNADataSource)dataSource;
              long contentStart = dlnaSource.getContentStart();
              LOG.I(TAG, "load position/length/contentStart = " + position + "/" + length + "/" + contentStart);
          }else{
              LOG.I(TAG, "load position/length = " + position + "/" + length);
          }
          //[htc]Add by OOD, for DLNA time seek, end
