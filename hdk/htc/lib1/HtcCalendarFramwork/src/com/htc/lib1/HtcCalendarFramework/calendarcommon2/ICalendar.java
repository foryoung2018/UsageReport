...
public class ICalendar {
    ...
    private static String normalizeText(String text) {
        ...
        //Fong, some case will create "=\n " from outlook when description is too long,
        //if replace "\n " directly, then can't be parsing.
        //so need replace "=\n" before replace "\n "
        text = text.replaceAll("=\n", "");
    ...
    private static Component parseComponentImpl(Component component,
        ...
        // split into lines
        text = text.replaceAll("ENCODING=QUOTED-PRINTABLE:=\n", "ENCODING=QUOTED-PRINTABLE:");
        text = text.replaceAll("\n=", "=");
        ...
        for (int i = 0; i < lines.length; i++) {
            try {
                if (lines[i]!=null) {
                    sb = new StringBuilder(lines[i].trim());
                }else {
                    continue;
                }
                // look ahead
                 while (i + 1 < lines.length && lines[i+1]!=null && lines[i+1].length()>0 && Character.isWhitespace(lines[i + 1].charAt(0))) {
                    sb.append(lines[i + 1].substring(1));
                    i++;
                }
                current = parseLine(sb.toString(), state, current);
                ...
                if (false) {
                    Log.v(TAG, "Cannot parse " + sb.toString(), fe);
                }
