...
public class FragmentedMp4Builder implements Mp4Builder {
    ...
    protected List<Track> sortTracksInSequence(List<Track> tracks, final int cycle, final Map<Track, long[]> intersectionMap) {
        ...
        Collections.sort(tracks, new Comparator<Track>() {
            public int compare(Track o1, Track o2) {
                ...
                long startSample1 = 0;
                long endSample1 = 0;
                if (startSamples1 != null) {
                	startSample1 = startSamples1[cycle];
                    // one based sample numbers - the first sample is 1
                    endSample1 = cycle + 1 < startSamples1.length ? startSamples1[cycle + 1] : o1.getSamples().size() + 1;
                } else {
                	Log.e(FragmentedMp4Builder.class.getName(), "Get startSamples1 from map is null");
				}
  
                long[] startSamples2 = intersectionMap.get(o2);
                long startSample2 = 0;
                long endSample2 = 0;
                if (startSamples2 != null) {
                	startSample2 = startSamples2[cycle];
                    // one based sample numbers - the first sample is 1
                    endSample2 = cycle + 1 < startSamples2.length ? startSamples2[cycle + 1] : o2.getSamples().size() + 1;
                } else {
                	Log.e(FragmentedMp4Builder.class.getName(), "Get startSamples2 from map is null");
				}
     ...
     * <ul>
     * <li>{@link #createMvhd(com.htc.lib1.htcmp4parser.googlecode.mp4parser.authoring.Movie) mvhd}</li>
     * <li>{@link #createMvex(com.htc.lib1.htcmp4parser.googlecode.mp4parser.authoring.Movie)  mvex}</li>
     * <li>a {@link #createTrak(com.htc.lib1.htcmp4parser.googlecode.mp4parser.authoring.Track, com.htc.lib1.htcmp4parser.googlecode.mp4parser.authoring.Movie)  trak} for every track</li>
     * </ul>
     *
     * @param movie the concerned movie
    ...
    protected Box createTfra(Track track, IsoFile isoFile) {
        ...
        for (Box box : boxes) {
            if (box instanceof MovieFragmentBox) {
                ...
                for (int i = 0; i < trafs.size(); i++) {
                    ...
                    final TrackFragmentHeaderBox tfHeaderBox = (traf != null) ? traf.getTrackFragmentHeaderBox() : null;
                    final TrackMetaData trackMetaData = (track != null) ? track.getTrackMetaData() : null;
                    
                    if (tfHeaderBox != null && trackMetaData != null && tfHeaderBox.getTrackId() == trackMetaData.getTrackId()) {
                        ...
                        for (int j = 0; j < truns.size(); j++) {
                            ...
                            for (int k = 0; k < trun.getEntries().size(); k++) {
                                ...
                                if (k == 0 && trun.isFirstSampleFlagsPresent()) {
                                    ...
                                    List<MovieExtendsBox> mvexs = null;
                                    final MovieBox movieBox = isoFile.getMovieBox();
                                    if (movieBox != null) {
                                    	mvexs = movieBox.getBoxes(MovieExtendsBox.class);
        ...
        if (track != null && track.getTrackMetaData() != null) {
     ...
     * isofile. Uses {@link #createTfra(com.htc.lib1.htcmp4parser.googlecode.mp4parser.authoring.Track, com.htc.lib1.htcmp4parser.coremedia.iso.IsoFile)}
     ...
     * by calling {@link #createTrex(com.htc.lib1.htcmp4parser.googlecode.mp4parser.authoring.Movie, com.htc.lib1.htcmp4parser.googlecode.mp4parser.authoring.Track)}
