...
import com.htc.lib1.htcmp4parser.googlecode.mp4parser.boxes.mp4.objectdescriptors.*;
...
public class AACTrackImpl extends AbstractTrack {
    ...
    public AACTrackImpl(InputStream inputStream) throws IOException {
        ...
        double packetsPerSecond = (double)mSampleRate / 1024.0;
        ...
        audioSampleEntry.setSampleRate(mSampleRate);
        ...
        final Integer samplingFrequencyIndex = samplingFrequencyIndexMap.get(mSampleRate);
        if (samplingFrequencyIndex != null) {
        	final int sampleRate = samplingFrequencyIndex;
        	audioSpecificConfig.setSamplingFrequencyIndex(sampleRate);
        } else {
        	Log.e(AACTrackImpl.class.getName(), "Error, Get sample rate is null in AACTrackImpl()");
        }
        ...
        trackMetaData.setTimescale(mSampleRate); // Audio tracks always use samplerate as timescale
    ...
    private boolean readVariables() throws IOException {
        ...
        final Integer sampleRate = samplingFrequencyIndexMap.get(brb.readBits(4));
        if (sampleRate != null) {
        	mSampleRate = sampleRate;
        } else {
        	Log.e(AACTrackImpl.class.getName(), "Error, Get sample rate is null in readVariables()");
        }
