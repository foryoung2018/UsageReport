...
// HTC::
static const SkUnichar WHITE_SQUARE = 0x25A1;
// ::HTC
...
uint16_t SkGlyphCache::unicharToGlyph(SkUnichar charCode) {
    ...
    if (rec.fID == id) {
    	...
    	// HTC::, using WHITE_SQUARE to replace un-found character
    	uint16_t resultID = fScalerContext->charToGlyphID(charCode);
    	if (0 == resultID)
    	{
    		resultID = fScalerContext->charToGlyphID(WHITE_SQUARE);
    	}
    	return resultID;
        // ::HTC
    }
...
const SkGlyph& SkGlyphCache::getUnicharMetrics(SkUnichar charCode,
    ...
    if (rec->fID != id) {
        ...
        id = SkGlyph::MakeID(resultID, x, y);
        // ::HTC
        rec->fGlyph = this->lookupMetrics(id, kFull_MetricsType);
    } else {
        RecordHashSuccess();
