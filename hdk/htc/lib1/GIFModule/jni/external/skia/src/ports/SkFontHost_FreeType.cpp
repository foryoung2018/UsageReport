...
//printf("fopen(%s)\n", keyString.c_str());
...
#ifdef DYNA_EMOJI
    char  fontfile[1024];
    const char* src = typeface->getFileName();
    size_t length = 0;
    if (src) {
		size_t size = strlen(src);
		length = SkMin32(size, sizeof(fontfile));
		memcpy(fontfile, src, length);
	}

    int32_t isDynaEmoji = -1; //1 is emoji, -1 is not emoji, 0 is unknown
    if (length) {
        fontfile[length] = '\0';
        isDynaEmoji = IsDynaEmoji(fontfile);
    }
    rec->isDynaEmoji = isDynaEmoji;
#endif
    ...
    if (err) {    // bad filename, try the default font
        ...
        SkASSERT(rec->fFace);
#ifdef DYNA_EMOJI
        if(rec->isDynaEmoji > 0)
        {
            rec->p_emoji = InitEmojiFont(fontfile);
        }
        else
        {
            rec->p_emoji = NULL;
...
SkScalerContext_FreeType::SkScalerContext_FreeType(SkTypeface* typeface,
    ...
    // +HTC, disable bold with CJK fonts (start)
    // Compare font family name, if it is "Droid Sans Fallback", do not embolden.
    fEnableBoldFace = true;
    if (fFace)
    {
        SkString faceFamilyName(fFace->family_name);
        SkString disableFakeName1("Droid Sans Fallback");
        SkString disableFakeName2("MotoyaLMaru");
        SkString disableFakeName3("MotoyaLCedar");
        if(faceFamilyName.equals(disableFakeName1) ||
            faceFamilyName.equals(disableFakeName2) ||
            faceFamilyName.equals(disableFakeName3))
        {
            fEnableBoldFace = false;
        }
    }
    // +HTC, disable bold with CJK fonts (end)
...
void SkScalerContext_FreeType::generateMetrics(SkGlyph* glyph) {
    ...
    switch ( fFace->glyph->format ) {
      ...
      case FT_GLYPH_FORMAT_BITMAP:
        if ((fRec.fFlags & kEmbolden_Flag) && !(fFace->style_flags & FT_STYLE_FLAG_BOLD)) {
            // +HTC, disable bold with CJK fonts (start with {})
            if (fEnableBoldFace)  // true:if family name is NOT "Droid Sans Fallback".
            {
...
void SkScalerContext_FreeType::generateImage(const SkGlyph& glyph) {
    ...
    if(fFaceRec->isDynaEmoji > 0)
    {
        ...
        {
            if( DynaEmojiTTF_Glyph(fFaceRec->p_emoji, (unsigned char*)glyph.fImage, glyph.fWidth, glyph.fHeight, glyph.rowBytes(), glyph.getGlyphID(fBaseGlyphCount) ) )
            {
                SkDebugf("generate DynaEmoji fail ....\n");
                memset(glyph.fImage, 0, glyph.rowBytes() * glyph.fHeight);
                return;
            }
        }
        else if (glyph.fMaskFormat == SkMask::kA8_Format)
        {
            SkGlyph tmpGlyph = glyph;
            tmpGlyph.fMaskFormat = SkMask::kARGB32_Format;
            unsigned size = tmpGlyph.computeImageSize();
            tmpGlyph.fImage = sk_malloc_throw(size);
            if (tmpGlyph.fImage)
            {
                if(DynaEmojiTTF_Glyph(fFaceRec->p_emoji, (unsigned char*)tmpGlyph.fImage, tmpGlyph.fWidth, tmpGlyph.fHeight, tmpGlyph.rowBytes(), tmpGlyph.getGlyphID(fBaseGlyphCount) ) )
                {
                    SkDebugf("generate DynaEmoji fail ....\n");
                    memset(glyph.fImage, 0, glyph.rowBytes() * glyph.fHeight);
                    return;
                ...
                SkBitmap bmp32, bmp8;
                bmp32.setConfig(SkBitmap::kARGB_8888_Config, tmpGlyph.fWidth, tmpGlyph.fHeight, tmpGlyph.rowBytes());
                bmp32.setPixels(tmpGlyph.fImage);
                bmp32.extractAlpha(&bmp8);
                bmp8.copyPixelsTo(glyph.fImage, glyph.computeImageSize(), glyph.rowBytes());

                sk_free(tmpGlyph.fImage);
            }
        }
        return;
    }
