...
#ifdef DYNA_EMOJI //HTC, support color emoji (support shadow)
    if (fMaskFilter && (glyph->fMaskFormat == SkMask::kARGB32_Format))
    {
        //SkDebugf("SkScalerContext::getMetrics change mask format");
...
#ifdef DYNA_EMOJI //HTC, support color emoji (support shadow)
        if (fMaskFilter && (tmpGlyph.fMaskFormat == SkMask::kARGB32_Format))
        {
            const_cast<SkGlyph&>(tmpGlyph).fMaskFormat = fRec.fMaskFormat;
        }
#endif
        glyph = &tmpGlyph;
    }
