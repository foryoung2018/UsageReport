...
static MergeAAProc find_merge_aa_proc(SkMask::Format format) {
    switch (format) {
        ...
        //++ [HTC][U15][Arthur_Hung][20130807]
        // ColorEmoji glyph will be RGBA format which will introduce a unsupported mask format and get NULL mergeProc then cause crash.
#ifdef DYNA_EMOJI
...
void SkAAClipBlitter::blitMask(const SkMask& origMask, const SkIRect& clip) {
    ...
    //++ [HTC][U15][Arthur_Hung][20130807]
    // unsupported mask format will get NULL mergeProc then cause crash.
    if (mergeProc == NULL) {
        return;
    }
    //-- [HTC][U15][Arthur_Hung][20130807]
