...
#ifdef DYNA_EMOJI
    virtual const char* getFileName(const FontIdentity&) const SK_OVERRIDE;
#endif
...
///////////////////////////////////////////////////////////////////////////////
#ifdef DYNA_EMOJI
#define ANDROID_EMOJI_FILE "/system/fonts/AndroidEmoji.ttf"
#define REPLACE_EMOJI_FILE "/system/fonts/AndroidEmoji-htc.ttf"
#endif
...
#ifdef DYNA_EMOJI
            if (strcmp(filename.c_str(), ANDROID_EMOJI_FILE) == 0) {
                SkString replaceName(REPLACE_EMOJI_FILE);
                if (has_font(fFonts, replaceName)) {
...
#ifdef DYNA_EMOJI
            if (strcmp(filename.c_str(), ANDROID_EMOJI_FILE) == 0) { // ...well...
                fontRec.fFileName = REPLACE_EMOJI_FILE;
            } else {
...
#ifdef DYNA_EMOJI
const char* SkFontConfigInterfaceAndroid::getFileName(const FontIdentity& identity) const {
	return identity.fString.c_str();
}
#endif
...
static HB_UnicodeMapping HB_UnicodeMappingArray[] = {
    ...
    // +[HTC][20130716], The first tamil character in customized font AnjalTamilMN
    // is 0x0B83, but DroidSansTamil is 0x0B82, fix this code point which allow
    //finding correct font file, so Tamil shaping can behave normally.
    {HB_Script_Tamil,      HB_SCRIPT_TAMIL,       0x0B83},
    // -[HTC]
